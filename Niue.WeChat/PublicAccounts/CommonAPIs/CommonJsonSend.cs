...
namespace Niue.WeChat.PublicAccounts.CommonAPIs
{
    public static class CommonJsonSend
    {
        ...
        public static WxJsonResult Send(string accessToken, string urlFormat, object data, CommonJsonSendType sendType = CommonJsonSendType.POST, int timeOut = Config.TIME_OUT, bool checkValidationResult = false, JsonSetting jsonSetting = null/*, int retry40001ErrorTimes = 0*/)
        {
            ...
            try
            {
                result = Core.CommonAPIs.CommonJsonSend.Send(accessToken, urlFormat, data, sendType, timeOut,
        ...
        {
            return Core.CommonAPIs.CommonJsonSend.Send<T>(accessToken, urlFormat, data, sendType, timeOut, checkValidationResult, jsonSetting);
        }
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        [Obsolete("此方法已过期，请使用Core.CommonAPIs.CommonJsonSend.SendAsync()方法")]
        ...
        /// <returns></returns>
        [Obsolete("此方法已过期，请使用Core.CommonAPIs.CommonJsonSend.SendAsync<T>()方法")]
