...
using Niue.WeChat.PublicAccounts.AdvancedAPIs.UserTag.UserTagJson;
using Niue.WeChat.PublicAccounts.CommonAPIs;

namespace Niue.WeChat.PublicAccounts.AdvancedAPIs.UserTag
{
    public class UserTagApi
    {
        ...
        public static TagJson Get(string accessTokenOrAppId)
        {
            return ApiHandlerWapper.TryCommonApi(accessToken =>
            {
                ...
                return Core.Utilities.HttpUtility.Get.GetJson<TagJson>(url);
        ...
        public static async Task <CreateTagResult> CreateAsync(string accessTokenOrAppId,string name, int timeOut = Config.TIME_OUT)
        {
            return await ApiHandlerWapper.TryCommonApiAsync( accessToken =>
            {
                ...
                return Core.CommonAPIs.CommonJsonSend.SendAsync<CreateTagResult>(accessToken, urlFormat, data, timeOut: timeOut);
            }, accessTokenOrAppId);
        }
        /// <summary>
        /// 【异步方法】获取公众号已创建的标签
        ...
        public static async Task<TagJson> GetAsync(string accessTokenOrAppId)
        {
            return await ApiHandlerWapper.TryCommonApiAsync( accessToken =>
            {
                var urlFormat = "https://api.weixin.qq.com/cgi-bin/tags/get?access_token={0}";
                var url = string.Format(urlFormat, accessToken);
                return Core.Utilities.HttpUtility.Get.GetJsonAsync<TagJson>(url);
        ...
        public static async Task<UserTagJsonResult> GetAsync(string accessTokenOrAppId, int tagid,string nextOpenid="", int timeOut = Config.TIME_OUT)
        {
            return await ApiHandlerWapper.TryCommonApiAsync( accessToken =>
            {
                ...
                return Core.CommonAPIs.CommonJsonSend.SendAsync<UserTagJsonResult>(accessToken, urlFormat, data, CommonJsonSendType.POST, timeOut);
            }, accessTokenOrAppId);
        }
        /// <summary>
        /// 【异步方法】批量为用户打标签
        ...
        public static async Task<UserTagListResult> UserTagListAsync(string accessTokenOrAppid,string openid,int timeOut = Config.TIME_OUT)
        {
            return await ApiHandlerWapper.TryCommonApiAsync( accessToken =>
            {
                ...
                return Core.CommonAPIs.CommonJsonSend.SendAsync<UserTagListResult>(accessToken, urlFormat, data, CommonJsonSendType.POST, timeOut);
            }, accessTokenOrAppid);
        }
        #endregion
    }
