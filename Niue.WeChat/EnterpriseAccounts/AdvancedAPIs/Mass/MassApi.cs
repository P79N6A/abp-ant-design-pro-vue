...
namespace Niue.WeChat.EnterpriseAccounts.AdvancedAPIs.Mass
{
    /// <summary>
    /// 发送消息
    /// </summary>
    public static class MassApi
    {
        ...
        public static MassResult SendMpNews(string accessToken, string toUser, string toParty, string toTag, string agentId, List<MpNewsArticle> articles, int safe = 0, int timeOut = Config.TIME_OUT)
        {
            var data = new
            {
                ...
                mpnews = new
                {
                    articles = articles.Select(z => new
                    {
                        z.title, z.thumb_media_id, z.author, z.content_source_url, z.content, z.digest, z.show_cover_pic
                    }).ToList()
                },
                safe
            };
        ...
        public static async Task<MassResult> SendTextAsync(string accessToken, string toUser, string toParty, string toTag, string agentId, string content, int safe = 0, int timeOut = Config.TIME_OUT)
        {
            ...
            return await Core.CommonAPIs.CommonJsonSend.SendAsync<MassResult>(accessToken, URL_FORMAT, data, CommonJsonSendType.POST, timeOut);
        }
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static async Task<MassResult> SendMpNewsAsync(string accessToken, string toUser, string toParty, string toTag, string agentId, List<MpNewsArticle> articles, int safe = 0, int timeOut = Config.TIME_OUT)
        {
            var data = new
            {
                touser = toUser,
