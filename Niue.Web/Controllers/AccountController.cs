...
using Niue.Web.Controllers.Results;
using Niue.Web.Models.Account;

namespace Niue.Web.Controllers
{
    public class AccountController : NiueControllerBase
    {
        ...
        private async Task<AbpLoginResult<Tenant, User>> GetLoginResultAsync(string usernameOrEmailAddress, string password, string tenancyName)
        {
            ...
            switch (loginResult.Result)
            {
                case AbpLoginResultType.Success:
                    return loginResult;
                default:
                    throw CreateExceptionForFailedLoginAttempt(loginResult.Result, usernameOrEmailAddress, tenancyName, loginResult.HashPassword);
            }
        }
        ...
        private Exception CreateExceptionForFailedLoginAttempt(AbpLoginResultType result, string usernameOrEmailAddress, string tenancyName, string hashPassword)
        {
            switch (result)
            {
                case AbpLoginResultType.Success:
                ...
                case AbpLoginResultType.InvalidUserNameOrEmailAddress:
                    return new UserFriendlyException("用户名不存在");
                case AbpLoginResultType.InvalidPassword:
                    return new UserFriendlyException("密码错误");
                case AbpLoginResultType.InvalidTenancyName:
                    return new UserFriendlyException(L("LoginFailed"), L("ThereIsNoTenantDefinedWithName{0}", tenancyName));
                case AbpLoginResultType.TenantIsNotActive:
                    return new UserFriendlyException(L("LoginFailed"), L("TenantIsNotActive", tenancyName));
                case AbpLoginResultType.UserIsNotActive:
                    return new UserFriendlyException("用户已被冻结，请联系管理员。");
                case AbpLoginResultType.UserEmailIsNotConfirmed:
                    return new UserFriendlyException("邮箱地址尚未验证"); //TODO: localize message
        ...
        public virtual async Task<ActionResult> Register(RegisterViewModel model)
        {
            try
            {
                ...
                if (model.IsExternalLogin)
                {
                    ...
                    model.Password = Core.Users.User.CreateRandomPassword();
                ...
                if (user.IsActive)
                {
                    ...
                    if (loginResult.Result == AbpLoginResultType.Success)
                    {
                        ...
                        return Redirect(Url.Action("Index", "Cms"));
                    }
        ...
        public virtual async Task<ActionResult> ExternalLoginCallback(string returnUrl, string tenancyName = "")
        {
            ...
            switch (loginResult.Result)
            {
                case AbpLoginResultType.Success:
                    ...
                    if (string.IsNullOrWhiteSpace(returnUrl))
                    {
                        returnUrl = Url.Action("Index", "Cms");
                    }
